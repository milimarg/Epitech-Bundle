...
def display_fit_validity(chi, freedom):
    has_printed = False

    print("Fit validity:\t\t", end='')
    for i in range(len(distribution_table[1])):
        if chi < distribution_table[freedom][i]:
            if i == 0:
                print(f"P > {distribution_table[0][i]}%")
                has_printed = True
                break
            print(f"{distribution_table[0][i]}% < P < {distribution_table[0][i - 1]}%")
            has_printed = True
            break
    if not has_printed:
        print(f"P < {distribution_table[0][-1]}%")
